...
public class Base64 {
    ...
    public static byte[] decode(String encoded, int offset, int length) throws IOException {
        ...
        for (i = 0, decodedLen = 0; i < length; i += 4, decodedLen += 3) {
            decodeQuantum(encoded.charAt(offset + i), encoded.charAt(offset + i + 1),
                    encoded.charAt(offset + i + 2), encoded.charAt(offset + i + 3), decoded,
                    decodedLen);
        }
     ...
     */
    private static void decodeQuantum(char in1, char in2, char in3, char in4, byte[] out,
	 ...
	 * @return
	 */
    public static InputStream baseToInputStream(final String base64){  
	    ByteArrayInputStream stream = null;
		try { 
		    byte[] bytes1 = decode(base64);  
    ...
    public static void main(String[] args) {
    	String ccc="eyJhIjoiMTc2MDYwNTkxMzkiLCJiIjoiMTUzNzU3MDc1MjMifQ==";
		try {
			System.out.println(new String(Base64.decode(ccc)));
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
