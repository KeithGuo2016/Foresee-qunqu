...
var init = function (W, H, WHRate) {
    ...
    that.loadImage = (src, width, height, isRotate) => {
      ...
      let size = cropperUtil.getAdjustSize(W, H, width, height)
        ...
        let newHeight= size.height*0.7;
        if(newWidth/newHeight>=WHRate){
            newWidth = WHRate*newHeight;
        }else{
            newHeight = newWidth/WHRate;
        }
        let xGap = (size.width - newWidth)/2
        let yGap = (size.height - newHeight)/2;
    ...
    that.drawLines = function (cropperMovableItems, imageInfo, callback) {
        ...
        orderedDots.push(cropperMovableItems['center'])
    ...
    that.setupMoveItem = (key, changedTouches, imageInfo, callback) => {
      ...
      let size = cropperUtil.getAdjustSize(W, H, imageInfo.width, imageInfo.height)
        ...
        if (changedTouches.length == 1) {
            ...
            //中心点的话，特殊判断
            if(key == "center"){
                let minX = (cropperMovableItems['topright'].x-cropperMovableItems['topleft'].x)/2;
                let minY = (cropperMovableItems['bottomright'].y-cropperMovableItems['topright'].y)/2;
            ...
            if (mode == 'rectangle') {
                ...
                if (key == 'topleft') {
                    let valueX = cropperMovableItems['topright'].x;
                    let valueY = cropperMovableItems['bottomleft'].y;
                    if(Math.abs((valueX-x)/(valueY-y))>=that.WHRate){
                        x = valueX - (valueY-y) * that.WHRate;
                    }else{
                        y = valueY - (valueX-x)/that.WHRate;
                    }
                    cropperMovableItems['bottomleft'].x = x
                    cropperMovableItems['topright'].y = y
                    cropperMovableItems['center'].x = cropperMovableItems['topleft'].x+ (cropperMovableItems['topright'].x-cropperMovableItems['topleft'].x)/2
                    cropperMovableItems['center'].y = cropperMovableItems['bottomright'].y+ (cropperMovableItems['topright'].y-cropperMovableItems['bottomright'].y)/2
                }
                else if (key == 'topright') {
                    let valueX = cropperMovableItems['topleft'].x;
                    let valueY = cropperMovableItems['bottomright'].y;
                    if(Math.abs((x- valueX)/(valueY-y))>=that.WHRate){
                        x = (valueY-y) * that.WHRate + valueX;
                    }else{
                        y = valueY - (x- valueX)/that.WHRate;
                    }
                    ...
                    let valueX = cropperMovableItems['bottomright'].x;
                    let valueY = cropperMovableItems['topleft'].y;
                    if(Math.abs((valueX-x)/(y - valueY))>=that.WHRate){
                        x = valueX - (y - valueY) * that.WHRate;
                    }else{
                        y = valueY + (valueX-x)/that.WHRate;
                    }
                    ...
                    let valueX = cropperMovableItems['bottomleft'].x;
                    let valueY = cropperMovableItems['topright'].y;
                    if(Math.abs((x-valueX)/(y-valueY))>=that.WHRate){
                        x = (y-valueY) * that.WHRate + valueX;
                    }else{
                        y = valueY - (valueX-x)/that.WHRate;
                    }
                    ...
                    let valueX = cropperMovableItems['center'].x;
                    let valueY = cropperMovableItems['center'].y;
                    for (let key in cropperMovableItems) {
                        if(key!="center"){
                            cropperMovableItems[key].x += x - valueX;
                            cropperMovableItems[key].y += y - valueY;
                        }
                    }
                }
            }
